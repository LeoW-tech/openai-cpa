...
class RegisterTokenWaitHistoryTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_record_token_wait_history_patches_duration_and_event(self):
        ...
        with patch.object(register, "_history_patch") as history_patch:
            with patch.object(register, "_history_event") as history_event:
                register._record_token_wait_history(run_ctx, wait_seconds=30)

        history_patch.assert_called_once_with(run_ctx, token_wait_duration_ms=30000)
        history_event.assert_called_once_with(
            run_ctx,
            ...
            ok_flag=True,
            message="30s",
            snapshot={"wait_seconds": 30, "wait_duration_ms": 30000},
        )
